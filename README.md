...
**Soulbound Tokens** : Access to premium websites, content, or communities. These soulbound tokens (SBTs) will have a wide array of purposes and will be held by unique crypto wallets called Genies.
...
The backend canister is going to store the information about the users of Segenie. It will store their username(unique), name and bio. Also this canister is going to store the users achievements that they have achieved.

Each achievement is going to have the following information stored in the canister:
...
issuer - the creator of the achievement
```

_Note this may be changed a bit in the future, since we may add some additional fields._

### Images
You may notice that in the listed information we don’t mention the image for the achievement. That is because we have a decision to make about that. The images can be stored on IPFS, in which case we would need an additional filed in the achievement for the CID. The other option is to store the images directly on the IC.
...
### Achievements
For now one of the main ideas of achievements is to restrict access to certain websites to only people that have the required achievements. The decision of who can access the website needs to be made by the website owners. They have to define the rules about accessing their website.

We have tought of multiple ways to achieve this, but the following is the best solution that we have found.

### Restricting certain websites to certain users
We are going to store information about restrictions of the certain websites on the  IC. For this we are going to add additional endpoints to the backend canister. So it will look like the following:

_website_domain - the domain of the website is going to be the key for searching the required achievements_
_achievements - this is going to be a vector of different achievements that the user must have to acces the website_

**But how are we going to verify that this is set by the website owner?**
...
This way the access will be restricted only if the users visit the website on Segenie, if they visit it using any other browser the access will not get restricted. Currently I have an idea for fixing this issue, but before adding it to this document, I have to make sure it works.
...
The npm package is another way to restrict access to a website. It would have the job of querying the achievements for a certain user. It should have a function that could take the principal of the user that is trying to access the website and return the achievements of the user.
